import type { Metadata, Viewport } from "next";
import "@fontsource-variable/syne";
import "@fontsource-variable/manrope";
import "./globals.css";
import { siteDescription, siteOrigin } from "@/lib/site";

export const metadata: Metadata = {
  metadataBase: new URL(siteOrigin || "http://localhost:3000"),
  title: "U*TTU Studio — Look-Lock",
  description: siteDescription,
  applicationName: "U*TTU Studio",
  ...(siteOrigin ? { alternates: { canonical: "/" } } : {}),
  openGraph: {
    type: "website", locale: "fr_CH", siteName: "U*TTU Studio",
    title: "U*TTU Studio — Look-Lock", description: siteDescription,
    ...(siteOrigin ? { url: siteOrigin } : {}),
    images: [{ url: "/og.jpg", width: 1200, height: 630, alt: "U*TTU Studio — Look-Lock. Une identité. Un canon. Un frein." }],
  },
  twitter: { card: "summary_large_image", title: "U*TTU Studio — Look-Lock", description: siteDescription, images: ["/og.jpg"] },
  icons: { icon: "/icon.svg" },
};
export const viewport: Viewport = { themeColor: "#0A0A0B", colorScheme: "dark" };

export default function RootLayout({ children }: { children: React.ReactNode }) {
  return <html lang="fr-CH"><body><a className="skip-link" href="#contenu">Aller au contenu</a>{children}</body></html>;
}
